...
/// <summary>
/// Small runtime control surface.
/// Never calls RenderingDevice directly.
/// </summary>
...
vesselCamera.Toggled +=
	value =>
	{
		...
		_lodControl.ValueChanged +=
			value =>
			{
				...
				_pointQueryDiagnostic?.SetSpatialLod(
					_selectedLod);
		...
		Check(
			...
			.Toggled +=
				value =>
				{
					if (_pointQueryDiagnostic != null)
					{
						_pointQueryDiagnostic.Visible =
							value;
					}
				};
		...
		performancePanel.SetAnchorsAndOffsetsPreset(
			Control.LayoutPreset.BottomLeft);


		performancePanel.OffsetLeft =
			8;
		...
		_status.SetAnchorsAndOffsetsPreset(
			Control.LayoutPreset.BottomWide);


		_status.OffsetTop =
			-30;

		_status.OffsetBottom =
			0;
		...
		RuntimeWaveSettings settings)
	{
		if (settings == null ||
			_band.Selected < 0 ||
			_band.Selected >= settings.PowerLog10.Length)
		{
			return;
		}
		...
		_bandPower.Value =
			settings.PowerLog10[index];


		_bandEnabled.ButtonPressed =
			!settings.Disabled[index];
	...
	OceanDiagnosticCameraController.ViewMode mode)
{
		...
		double delta)
	{
		...
		if (_lightGizmo != null &&
			...
			_runtime.TryGetAnimatedWaveSurface(
				...
				out _,
				out _,
				out AnimatedWaveLodSlice lightSlice))
		{
			...
			_runtime.GetWaveSettingsSnapshot() is { } startup)
		{
			...
			_structure.Text =
				...
				$"{_runtime.RuntimeAnimatedWaveLodCount}; " +
				$"sampling ×" +
				$"{_runtime.RuntimeResolutionMultiplier:0.##}";
	...
	private void UpdateStatus()
	{
		...
		{
			_status.Text =
				"Ocean diagnostics: waiting for GPU";
		...
		{
			_status.Text =
				"Ocean diagnostics: waiting for wave settings";
		...
		if (nested &&
			_runtime.TryGetAnimatedWaveSurface(
				...
				out _,
				out _,
				out AnimatedWaveLodSlice outerSlice))
		{
			layoutStatus =
				$"Nested LOD0-" +
				$"{_runtime.RuntimeAnimatedWaveLodCount - 1} | " +
		...
		_status.Text =
			...
			$"LOD scale ×{_runtime.RuntimeLodScale:0.#} | " +

			$"scale alpha {_runtime.RuntimeLodScaleAlpha:0.00} | " +
			...
			$"H0 rev {_runtime.AppliedH0Revision}" +
		...
		else if (_runtime.TryGetAnimatedWaveSurface(
			index,
			out _,
			out _,
			out _,
			out AnimatedWaveLodSlice debugSlice))
		{
			_inset.PhysicalMetadata =
				$"size {debugSlice.WorldSize:0.###}m | " +
				$"texel {debugSlice.TexelWidth:0.###}m | " +
				$"λ {debugSlice.MinWavelength:0.###}-" +
				$"{debugSlice.MaxWavelength:0.###}m";
		}
	}
	...
	private static OptionButton Option(
		VBoxContainer parent,
		string label,
		params string[] items)
	{
